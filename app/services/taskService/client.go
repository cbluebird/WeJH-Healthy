...
func PutTestTask(t TaskType) {
	...
	_, err = task.AsynqClient.Enqueue(t1, asynq.ProcessIn(6*time.Hour), asynq.MaxRetry(10), asynq.Queue("healthy-check"), asynq.Timeout(120*time.Second))
	if err != nil {
		log.Println(err)
		return
	}
...
func PutRetryTask(t TaskType) {
	...
	_, err = task.AsynqClient.Enqueue(t1, asynq.ProcessIn(30*time.Minute), asynq.MaxRetry(5), asynq.Queue("healthy-retry"), asynq.Timeout(120*time.Second))
	if err != nil {
		log.Println(err)
		return
	}
